...
}function registerSignals(force){if(!force&&!!StorageHelper.maintenanceStartSignal){console.warn("[Opal.LocalStorage] %1: maintenance start signal already set!".arg(objectName))
}else{StorageHelper.maintenanceStartSignal=function(){maintenanceStarted()
}
}if(!force&&!!StorageHelper.maintenanceEndSignal){console.warn("[Opal.LocalStorage] %1: maintenance end signal already set!".arg(objectName))
}else{StorageHelper.maintenanceEndSignal=function(){maintenanceFinished()
}
}}visible:false
...
onAutoShowOnMaintenanceChanged:{if(autoShowOnMaintenance){registerSignals()
}}Connections{target:autoShowOnMaintenance?root:null
...
visible:false
}}]transitions:[Transition{from:"shown"
to:"hidden"
...
property:"visible"
duration:0
}}}]Component.onCompleted:{if(autoShowOnMaintenance){registerSignals()
}}}